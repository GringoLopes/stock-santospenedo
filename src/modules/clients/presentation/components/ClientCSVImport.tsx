...
import { useUsers } from "@/src/modules/auth/presentation/hooks/useUsers"
import { AlertCircle, CheckCircle, Copy, Download, FileText, Upload, Users, X } from "lucide-react"
...
export function ClientCSVImport() {
  ...
  const validateRow = (row: any, index: number): string | null => {
    ...
    if (row.code.length > 20) {
      return `Linha ${index + 1}: Código do cliente não pode exceder 20 caracteres`
    }
    ...
    if (row.client.length > 255) {
      return `Linha ${index + 1}: Nome do cliente não pode exceder 255 caracteres`
    }
    ...
    // Validar CNPJ (opcional, mas se fornecido deve ter formato válido)
    if (row.cnpj && row.cnpj.length > 0) {
      // CNPJ deve ter 14 dígitos
      if (row.cnpj.length !== 14) {
        return `Linha ${index + 1}: CNPJ deve ter 14 dígitos (apenas números)`
      }
      
      // Verificação básica de CNPJ válido (não todos iguais)
      if (/^(\d)\1{13}$/.test(row.cnpj)) {
  ...
  const importClients = async () => {
    ...
    if (!selectedUserId) {
      toast({
        ...
        description: "Por favor, selecione um usuário para vincular os clientes.",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (seenCodes.has(row.code)) {
          errors.push(`Linha ${processedCount + 1}: Código '${row.code}' duplicado no arquivo`)
        ...
        if (row.cnpj && seenCnpjs.has(row.cnpj)) {
          errors.push(`Linha ${processedCount + 1}: CNPJ '${row.cnpj}' duplicado no arquivo`)
  ...
  return (
    <Card>      
      <CardHeader>
        ...
        <CardDescription>
          Importe clientes em massa usando arquivos CSV ou TXT. Formato: Código;Nome;Cidade;CNPJ
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">    
        <div className="flex justify-between items-center">
          <p className="text-sm text-gray-600">Importe clientes no formato CSV/TXT</p>
              ...
              <div className="border rounded p-3 bg-gray-50 text-sm">
                ...
                {preview.map((item, index) => (
                  <div key={index} className="mb-1 font-mono text-xs">
                    ...
                    Cliente: <span>{item.client}</span> | 
                    Cidade: <span>{item.city}</span> |
                    CNPJ: <span>{item.cnpj || "N/A"}</span>
                  </div>
                ))}
              </div>
            )}
                ...
                {result.errors.length > 0 && result.errorDetails && (
                  <>
                    {result.errorDetails.duplicateCodes.length > 0 && (
                      <details>
                        <summary className="cursor-pointer font-semibold text-sm">
                          Ver códigos duplicados ({result.errorDetails.duplicateCodes.length})
                        </summary>
                        <div className="mt-2 space-y-1 text-sm max-h-40 overflow-y-auto">
                          {result.errorDetails.duplicateCodes.map((error, idx) => (
                    ...
                    {result.errorDetails.duplicateCnpjs.length > 0 && (
                      <details>
                        <summary className="cursor-pointer font-semibold text-sm">
                          Ver CNPJs duplicados ({result.errorDetails.duplicateCnpjs.length})
                        </summary>
                        <div className="mt-2 space-y-1 text-sm max-h-40 overflow-y-auto">
                          {result.errorDetails.duplicateCnpjs.map((error, idx) => (
                            <div key={idx} className="text-red-600">
                    ...
                    <div className="mt-4 text-sm">
                      ...
                      <ul className="list-disc pl-4 space-y-1 text-sm">
                        <li>Verifique se não há códigos de clientes duplicados</li>
                        <li>Certifique-se de que os CNPJs tenham 14 dígitos (apenas números)</li>
