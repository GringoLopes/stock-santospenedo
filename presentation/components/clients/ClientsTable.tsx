...
export function ClientsTable({ clients, loading, hasSearched, searchQuery, error, total, page, pageSize, onPageChange }: ClientsTableProps) {
      ...
      <Card>
        ...
        <CardContent className="px-4 sm:px-6">
          <Alert variant="destructive">
            ...
            <AlertDescription>Erro ao buscar clientes: {error.message || "Erro desconhecido"}</AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    )
        ...
        <CardContent className="px-4 sm:px-6">
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-gray-500 max-w-md text-sm sm:text-base">
              Use o formulário acima para buscar clientes por código, nome, cidade ou CNPJ.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="px-4 sm:px-6">
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-gray-500 max-w-md text-sm sm:text-base">
              Não foram encontrados clientes que correspondam à sua busca por "{searchQuery}". Tente usar termos
    ...
    <Card>
      <CardHeader className="px-4 sm:px-6">
        ...
        <div className="flex flex-col sm:flex-row  items-start sm:items-center justify-between">
          <div className="text-sm text-gray-600">
            <strong>{clients.length}</strong> cliente{clients.length !== 1 ? "s" : ""} encontrado
            {clients.length !== 1 ? "s" : ""} para: <strong className="break-all">"{searchQuery}"</strong>
          </div>
        </div>
      </CardHeader>
